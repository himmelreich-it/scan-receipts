...
class TestTUIIntegration:
    ...
    def test_startup_display_with_no_data(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert fs_adapter.count_receipt_files(config.incoming_folder) == 0
            assert fs_adapter.count_receipt_files(config.failed_folder) == 0
            assert fs_adapter.get_staging_info(config.csv_staging_file) is None
    ...
    def test_startup_display_with_files_and_staging(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert fs_adapter.count_receipt_files(config.incoming_folder) == 15
            assert fs_adapter.count_receipt_files(config.failed_folder) == 2

            staging_info = fs_adapter.get_staging_info(config.csv_staging_file)
