...
class TestEndToEndExtraction:
    ...
    @patch('ai_extraction.infrastructure.config.settings.anthropic_api_key', 'test_key')
    ...
    def test_complete_successful_extraction_workflow(self, mock_anthropic_class):
        ...
        assert result.data.amount == 45.67
        assert result.data.tax == 5.67
    ...
    def test_complete_error_handling_workflow(self, mock_anthropic_class):
        ...
        # Arrange
        mock_anthropic = Mock()
        mock_anthropic.messages.create.side_effect = RateLimitError("Rate limit exceeded")
        mock_anthropic_class.return_value = mock_anthropic
        
        facade = ExtractionFacade()
        
        # Act
        with patch('ai_extraction.infrastructure.config.settings.max_retries', 1):
            result = facade.extract_from_image(self.test_request)
