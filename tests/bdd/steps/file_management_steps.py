...
def step_given_file_system_permissions(context):
...
def step_given_folders_do_not_exist(context, folder_list):
...
def step_when_ensure_folder_structure(context):
    ...
    for folder_name, folder_path in context.folder_paths.items():
        ...
        context.results[folder_name] = result
...
def step_then_each_folder_writable(context):
...
def step_given_all_folders_exist_with_files(context):
    ...
    for folder_name, folder_path in context.folder_paths.items():
        ...
        context.test_files[folder_name] = test_file


@when('I begin analysis workflow')  # type: ignore
def step_when_begin_analysis_workflow(context):
...
def step_then_scanned_folder_empty(context):
...
def step_then_folders_retain_files(context, folder_list):
...
def step_given_folder_permission_denied(context, folder_name):
...
def step_when_try_ensure_folder_structure(context):
...
def step_then_receive_error_code(context, expected_error_code):
...
def step_then_error_includes_folder_path(context):
...
def step_given_all_folders_writable(context):
...
def step_given_test_receipt_file(context, filename):
...
def step_given_file_in_incoming(context):
...
def step_when_copy_to_scanned(context, target_filename):
...
def step_then_file_in_both_folders(context):
...
def step_then_scanned_file_has_name(context, expected_name):
...
def step_given_file_in_scanned(context, filename):
...
def step_when_move_to_imported(context, target_filename):
...
def step_then_file_only_in_imported(context):
...
def step_then_imported_file_has_name(context, expected_name):
    """Verify imported file has expected name."""
    assert context.target_file.name == expected_name, f"Expected {expected_name}, got {context.target_file.name}"


@given('a file operation encounters "{error_condition}"')  # type: ignore
def step_given_file_operation_error(context, error_condition):
...
def step_when_attempt_file_operation(context):
...
def step_then_receive_file_error_code(context, expected_error):
...
def step_given_test_files_known_content(context):
...
def step_given_file_known_content(context, filename):
...
def step_then_receive_64_char_hex(context):
...
def step_then_hash_matches_expected(context):
...
def step_given_empty_file(context, filename):
...
def step_then_receive_empty_hash(context):
...
def step_given_file_with_error(context, error_condition):
...
def step_when_try_generate_hash(context):
...
def step_then_receive_hash_error_code(context, expected_hash_error):
...
def step_given_all_folders_exist(context):
...
def step_given_folder_not_writable(context, folder_name):
...
def step_when_validate_folder_permissions(context):
...
def step_then_error_specifies_folder_path(context):
...
def step_given_incoming_folder_empty(context):
...
def step_when_check_files_to_process(context):
...
def step_then_system_continues_without_error(context):
...
def step_then_no_files_processed(context):
...
def step_given_imported_folder_has_files(context):
...
def step_when_any_system_operation_performed(context):
...
def step_then_imported_files_unchanged(context):
...
def step_given_failed_folder_has_error_logs(context):
...
def step_then_failed_folder_unchanged(context):
    ...
    for filename in context.existing_failed_files:
        ...
        if filename.endswith('.log'):
            content = file_path.read_text()
            assert content == "Error: File corrupted during processing", f"Log content should be unchanged"
        else:
            content = file_path.read_bytes()
            assert content == b"corrupted file content", f"File content should be unchanged"
