...
class TerminalUI:
    ...
    def display_menu(self, config: AppConfig) -> None:
        ...
        # Show "Re-run Analysis" if receipts.csv exists, otherwise "Run Analysis"
        analysis_text = (
            "Re-run Analysis" if config.csv_staging_file.exists() else "Run Analysis"
        )
        rprint(f"[1] {analysis_text}")
